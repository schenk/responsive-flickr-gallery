...
if (false) {
    ...
    add_filter('plugins_api_result', 'rfg_aaa_result', 10, 3);
    function rfg_aaa_result($res, $action, $args)
    {
        print_r($res);
        return $res;
    }
...
add_filter(
    ...
    'rfg_check_for_plugin_update'
);

define('FRG_SSL_VERIFY', false);
  
add_action('http_request_args', 'rfg_ssl_verify', 10, 2);
function rfg_ssl_verify($args, $url)
{
    $args['sslverify'] = FFG_SSL_VERIFY;
    return $args;
}

function rfg_check_for_plugin_update($checked_data)
{
    ...
    $request_string = rfg_prepare_request('basic_check', $request_args);
...
function my_plugin_api_call($def, $action, $args)
{
    ...
    $request_string = rfg_prepare_request($action, $args);
...
function rfg_prepare_request($action, $args)
{
    global $wp_version;
    $site_url = site_url();
